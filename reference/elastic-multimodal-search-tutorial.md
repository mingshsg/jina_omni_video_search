---
title: "Tutorial: Build multimodal search in Elasticsearch"
source: https://www.elastic.co/docs/solutions/search/multimodal-search/multimodal-search-tutorial
source_markdown: https://www.elastic.co/docs/solutions/search/multimodal-search/multimodal-search-tutorial.md
...
url: https://www.elastic.co/docs/solutions/search/multimodal-search/multimodal-search-tutorial
products:
...
# Tutorial: Build multimodal search in Elasticsearch
...
For concepts and deployment options, refer to [Multimodal search](https://www.elastic.co/docs/solutions/search/multimodal-search).
...
- Access to a multimodal `embedding` endpoint. This tutorial uses the preconfigured `.jina-embeddings-v5-omni-small` endpoint through the [Elastic Inference Service (EIS)](https://www.elastic.co/docs/explore-analyze/elastic-inference/eis).
...
## 1. Verify the multimodal inference endpoint

This example uses the preconfigured `.jina-embeddings-v5-omni-small` endpoint. Elastic recommends [Jina multimodal embeddings](/docs/explore-analyze/machine-learning/nlp/ml-nlp-jina#jina-multimodal-embeddings) for multimodal search.
...
The `_source` exclusion omits the Base64-encoded image value while returning the other source fields. A `match` query on a `semantic` field sends the text to the field's inference endpoint and compares the resulting embedding with the indexed image embeddings.
...
### Filter semantic image search results
...
## 5. Search the images with another image
...
The query searches the embeddings indexed for the `image` field and uses the inference endpoint from that field's mapping. The `embedding` query vector builder sends the raw image input to the endpoint to generate the query vector. `k` returns the three nearest neighbors. `num_candidates` compares the query vector against four approximate nearest-neighbor candidates per shard.
<dropdown title="Example response">
  ```json
  {
    "total": {
...
## 6. Search the images with a PDF
...
The input type tells the multimodal inference endpoint to process the binary as a PDF. The PDF is supplied as a Base64 data URL and embedded at query time.
<dropdown title="Example response">
  ```json
  {
    "total": {
...
- [Multimodal search](https://www.elastic.co/docs/solutions/search/multimodal-search)
- [`semantic` field type](https://www.elastic.co/docs/reference/elasticsearch/mapping-reference/semantic-field)
- [Jina multimodal embedding models](/docs/explore-analyze/machine-learning/nlp/ml-nlp-jina#jina-multimodal-embeddings)
- [inference endpoints](https://www.elastic.co/docs/explore-analyze/elastic-inference/inference-api)
